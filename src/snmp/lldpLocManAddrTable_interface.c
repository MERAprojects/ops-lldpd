...
int lldpLocManAddrTable_index_from_oid(netsnmp_index *oid_idx,
    ...
    if (err == SNMP_ERR_NOERROR) {
        mib_idx->lldpLocManAddrSubtype =
            *((long *)((uintptr_t)var_lldpLocManAddrSubtype.val.string));
...
_lldpLocManAddrTable_get_column(lldpLocManAddrTable_rowreq_ctx *rowreq_ctx,
    ...
    case COLUMN_LLDPLOCMANADDRLEN: {
        ...
        rc = lldpLocManAddrLen_get(rowreq_ctx, (long *)((uintptr_t)var->val.string));
    } break;
    case COLUMN_LLDPLOCMANADDRIFSUBTYPE: {
        var->type = ASN_INTEGER;
        var->val_len = sizeof(long);
        rc = lldpLocManAddrIfSubtype_get(rowreq_ctx, (long *)((uintptr_t)var->val.string));
    } break;
    case COLUMN_LLDPLOCMANADDRIFID: {
        var->type = ASN_INTEGER;
        var->val_len = sizeof(long);
        rc = lldpLocManAddrIfId_get(rowreq_ctx, (long *)((uintptr_t)var->val.string));
    } break;
