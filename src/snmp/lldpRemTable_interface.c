...
_lldpRemTable_get_column(lldpRemTable_rowreq_ctx *rowreq_ctx,
    ...
    case COLUMN_LLDPREMCHASSISIDSUBTYPE: {
        ...
        rc = lldpRemChassisIdSubtype_get(rowreq_ctx, (long *)((uintptr_t)var->val.string));
    } break;
    ...
    case COLUMN_LLDPREMPORTIDSUBTYPE: {
        var->type = ASN_INTEGER;
        var->val_len = sizeof(long);
        rc = lldpRemPortIdSubtype_get(rowreq_ctx, (long *)((uintptr_t)var->val.string));
    } break;
    ...
    case COLUMN_LLDPREMSYSCAPSUPPORTED: {
        ...
        int idx = 0;
        var->type = ASN_OCTET_STR;
        rc = lldpRemSysCapSupported_get(rowreq_ctx, (u_long *)((uintptr_t)var->val.string));
        var->val_len = 0;
        while (0 != mask) {
            ++idx;
            if (*((u_long *)((uintptr_t)var->val.string)) & mask)
                var->val_len = idx;
            mask = mask >> 8;
        }
    } break;
    case COLUMN_LLDPREMSYSCAPENABLED: {
        ...
        int idx = 0;
        var->type = ASN_OCTET_STR;
        rc = lldpRemSysCapEnabled_get(rowreq_ctx, (u_long *)((uintptr_t)var->val.string));
        var->val_len = 0;
        while (0 != mask) {
            ++idx;
            if (*((u_long *)((uintptr_t)var->val.string)) & mask)
